...
export const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  const getRelativeEchoTime = (id: string, fallback: string): string => {
    const tsNum = parseInt(id.replace('echo-', ''), 10);
    if (isNaN(tsNum)) return fallback;
    const diffMs = Date.now() - tsNum;
  ...
  const firstMetDate = new Date('2026-04-20');
  const now = new Date();
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          {featuredChapter ? (
            ...
            <div className="glass-panel p-8 rounded-3xl border border-dashed border-[#d4af37]/20 text-center py-12 space-y-4">
              <BookOpen className="w-12 h-12 text-[#a39780] mx-auto opacity-60" />
              <div className="space-y-1">
                <h4 className="text-lg font-display text-[#fff8e7]">No Chapters Recorded Yet</h4>
                <p className="text-xs text-[#a39780] max-w-sm mx-auto">
                  Your story is waiting to be written. Record your first special memory together!
                </p>
              </div>
              <button
                onClick={onOpenAddChapter}
                className="px-4 py-2 rounded-xl bg-[#d4af37]/10 hover:bg-[#d4af37]/25 text-[#d4af37] border border-[#d4af37]/30 text-xs font-semibold cursor-pointer transition"
              >
          ...
          <div className="space-y-3">
            ...
            {featuredSong ? (
              <div 
                ...
                className="glass-panel p-5 rounded-3xl border border-[#d4af37]/20 relative overflow-hidden cursor-pointer hover:border-[#d4af37]/50 transition group"
              >
                <div className="flex items-center space-x-4">
                  <div className="relative w-20 h-20 rounded-2xl overflow-hidden shrink-0 shadow-lg border border-[#d4af37]/30">
                    ...
                    />
                    <div className="absolute inset-0 bg-[#000000]/30 group-hover:bg-[#000000]/10 flex items-center justify-center text-[#d4af37] transition">
                      <Music className="w-7 h-7 text-[#d4af37] group-hover:scale-110 transition" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 overflow-hidden flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-[#d4af37] flex items-center space-x-1 opacity-80 group-hover:opacity-100 group-hover:translate-x-0.5 transition font-medium">
                        <span>Music Library</span>
                        <ArrowRight className="w-3 h-3" />
                      </span>
                    </div>
                    <h4 className="text-base font-display font-semibold text-[#fff8e7] truncate group-hover:text-[#d4af37] transition">
                      {featuredSong.title}
                    </h4>
              ...
              <div className="glass-panel p-6 rounded-3xl border border-dashed border-[#d4af37]/20 text-center py-8 space-y-3">
                <Music className="w-10 h-10 text-[#a39780] mx-auto opacity-60" />
                <div className="space-y-1">
                  <h4 className="text-sm font-display text-[#fff8e7]">No Tracks Added Yet</h4>
                  <p className="text-[11px] text-[#a39780]">
                    Build your shared soundtrack. Add a song that reminds you of each other!
                  </p>
                </div>
                <button
                  onClick={() => setActiveTab('music')}
                  className="px-3.5 py-1.5 rounded-lg bg-[#d4af37]/10 hover:bg-[#d4af37]/25 text-[#d4af37] border border-[#d4af37]/30 text-[11px] font-semibold cursor-pointer transition"
                >
                  Go to Music Tab
                </button>
              </div>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[9px] text-[#8c816d] shrink-0 ml-2 font-mono">{getRelativeEchoTime(echo.id, echo.timestamp)}</span>
                        </div>
